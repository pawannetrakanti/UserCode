...
void RAA_plot_yetkinCutEfficiency(Int_t radius = 3){
  ...
  cout<<"matched Data ntuple "<< entries << endl;
  
  for(long nentry = 0; nentry < entries; ++nentry ){

    if(nentry%500000 == 0) cout<<" nentry = "<<nentry<<endl;
  ...
  //entries = 1000;
  cout<<"Unmatched Data ntuple "<< entries << endl;
  for(long nentry = 0; nentry < entries; ++nentry ){
  ...
  //entries = 1000;
  // MC loop
  cout<<" looking at matched MC ntuple "<< entries << endl;
  for(long nentry = 0; nentry < entries; ++nentry){
  ...
  //entries = 1000;
  // MC loop
  cout<<" looking at unmatched MC ntuple : "<< entries << endl;
  for(long nentry = 0; nentry < entries; ++nentry){
  ...
  TFile *fout = new TFile(Form("Output/PbPb_CutEfficiency_YetkinCuts_matched_slantedlinecalopfpt_addingunmatched_exclusionhighertriggers_eMaxSumcand_A_R0p%d.root",radius),"RECREATE");
  fout->cd();
  ...
  // cCutRejection_Jet55->SaveAs("PbPb_YetkinCuts_Jet55_noJet65noJet80_eMaxSumcand_A_chMaxJtpt_calopfpt_rejection_including_unmatched.pdf");
  ...
  // cCutRejection_Jet65->SaveAs("PbPb_YetkinCuts_Jet65_noJet80_eMaxSumcand_A_chMaxJtpt_calopfpt_rejection_including_unmatched.pdf");
