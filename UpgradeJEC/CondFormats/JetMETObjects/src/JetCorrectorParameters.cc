...
void JetCorrectorParameters::printFile(const std::string& fFileName) const
{
  ...
  txtFile<<"{"<<definitions().nBinVar()<<std::setw(35);
  for(unsigned i=0;i<definitions().nBinVar();i++)
    txtFile<<definitions().binVar(i)<<std::setw(35);
  txtFile<<definitions().nParVar()<<std::setw(35);
  for(unsigned i=0;i<definitions().nParVar();i++)
    txtFile<<definitions().parVar(i)<<std::setw(35);
  txtFile<<std::setw(definitions().formula().size()+35)<<definitions().formula()<<std::setw(35);
  if (definitions().isResponse())
    txtFile<<"Response"<<std::setw(35);
  else
    txtFile<<"Correction"<<std::setw(35);
  txtFile<<definitions().level()<<"}"<<"\n";
  for(unsigned i=0;i<size();i++)
    {
      for(unsigned j=0;j<definitions().nBinVar();j++)
        txtFile<<record(i).xMin(j)<<std::setw(35)<<record(i).xMax(j)<<std::setw(35);
      txtFile<<record(i).nParameters()<<std::setw(35);
      for(unsigned j=0;j<record(i).nParameters();j++)
        txtFile<<record(i).parameter(j)<<std::setw(35);
      txtFile<<"\n";
    }
  txtFile.close();
}
