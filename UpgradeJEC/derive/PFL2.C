...
void perform_smart_fit(int nj, TGraphErrors * gabscor, TF1 * fabscor, int maxFitIter); 

double ptbins[] ={1,5,10,15,20,25,30,35,40,45,57,72,90,120,150,200,300,400,550,750,1000,1500};
const int npt = sizeof(ptbins)/sizeof(double) - 1;

const double ketacut=1.30;
double etabins[] = {-1.30, -1.00, -0.50, 0.00, 0.50, 1.00, +1.30};
...
int PFL2(const int scene=5,const char *ctype="pf")
{
  bool iSave=false;
  double minx=5.0;
  ...
  if( scene == 3 ){
    fin = new TFile("input/jra_ecal-aged-hcal-aged-sipm-nonaged_scenario3.root","r");
    fl3 = new TFile("l3pf_scene3_input.root","r");
  }else{
  ...
  for(int i=njmin; i<njmax; i++){
    ...
    for(int k=0; k<neta; k++){ //! eta    
      ...
      if (npoints > 0){
	xmin = TMath::Max(gabscor[i][k]->GetX()[0],10.0);
	...
	fabscor->SetParLimits(4,0,100);
      ...
      perform_smart_fit(i, gabscor[i][k], fabscor, 30);
    ...
    for(int k=0; k<neta; k++){ //! eta  
      double  xmin    = TMath::Max(10.0,grelcor[i][k]->GetX()[0]);
  ...
  //return 0;
   ...
   for(int nj=njmin; nj<njmax; nj++){
     ...
     //c98[nj]->Divide(3,3,0,0);
     c98[nj]->Divide(3,2,0,0);
     ipad=0;
     for(int ie=0;ie<neta;ie++){      
