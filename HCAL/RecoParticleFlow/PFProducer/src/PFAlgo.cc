...
/// Swagata start ///            
double get_HO_weight(double Energy, double EcalEnergy, double particleEta) {
  double HO_wt = -999;
  double xx = Energy;
  if (EcalEnergy>1.0) {
    xx = max(Energy, 35.0);
    HO_wt = 0.705+(-7.947e+05+6.755e+06/sqrt(xx))*exp(-xx/2.682)-6324*exp(-xx*xx/128.1) ;
  }

  if (EcalEnergy<1.0) {
    xx = max(Energy, 6.0);
    HO_wt = 1.087+(-0.3972+0.2951/sqrt(xx))*exp(-xx/224.1)+1.881e+05*exp(-xx*xx/1.869) ;
  }
  return HO_wt;
}
/// Swagata end ///              
...
void PFAlgo::processBlock( const reco::PFBlockRef& blockref,
  ...
  ///Swagata start/// 
  double partEnergy = 0.0;  /// Energy of track
  ///Swagata end///                                                                                                                                                                       
  ...
  for(unsigned iEle=0; iEle<elements.size(); iEle++) {
	...
	std::cout << elements[iTrack] << std::endl; 
      ...
      ///Swagata start
      if( useHO_ )partEnergy = trackMomentum; 
      ///Swagata end
...
//       cerr<<"not ready for navigation in the HF!"<<endl;
  ...
  for(unsigned i=0; i<hcalIs.size(); i++) {
    ...
    case 0 : wt1 = 0; break;     
    case 1 : wt1 = 1; break;
    case 2 : wt1 = get_HO_weight(partEnergy, totalEcal, 0.0) ; break;
    default : wt1 = 1.0; break;
    }
    totalHO=totalHO*wt1;
    if( useHO_ )totalHcal += totalHO; // Commented now .. 11/03/2015 3:45 pm
    ///// Swagata end /////
  ...
  for(unsigned ihcluster=0; ihcluster<hcalIs.size(); ihcluster++) {
    ...
    double totalHcal =  hclusterRef->energy();
    ///// Swagata start /////
    // std::cout <<"iTagHO "<< tagHO_ <<std::endl;
    double wt2 = 1.0;
    switch ( tagHO_ ) {
    case 0 : wt2 = 0; break;     
    case 1 : wt2 = 1; break;
    case 2 : wt2 = get_HO_weight(partEnergy, totalEcal, 0.0) ; break;
    default: wt2=1.0;  break;
    }
    totalHO=totalHO*wt2;
    ///// Swagata end /////                          
