...
float binw   = 2;
//! used for fit
double ptbins[]   = {15,30,40,70,110,150,190,250,300,400,1000};
const int b1  = sizeof(ptbins)/sizeof(Double_t) - 1;
const int nbins = b1;

float minval_phi = -3.14;
float maxval_phi =  3.14;
float binw_phi   = 8.72222222222222288e-02;
double phibins[] ={-3.141, -2.700, -2.100, -1.500, -0.900, -0.300,
		   0.300, 0.900, 1.500, 2.100, 2.700, 3.141};  
const int b2  = sizeof(phibins)/sizeof(Double_t) - 1;
const int nphi = b2;
...
int JetEnergyResolution_phi(const char *rphi = "phi2")
{
  ...
  float kphicut=2.0;
  if(strcmp(rphi,"phi3.0")==0)kphicut=3;
  bool iSigma=false;
  ...
  for(int in=0; in<kfiles; in++){
    fin_pbpb[in] = new TFile("input/JetResponse_histos_pp_alljetalgos.root","r");
	...
	TH3F *hreco = (TH3F*)fin_pbpb[in]->Get(Form("hrescrphi_genm%d_%d",nj,icen));
	TH3F *hraw  = (TH3F*)fin_pbpb[in]->Get(Form("hresrrphi_genm%d_%d",nj,icen));
	...
	for(int ip=0; ip<nbins; ip++){	  	
	  ...
	  for(int ie=0; ie<nphi; ie++){

	    int lphi = hreco->GetXaxis()->FindBin(phibins[ie]);  //(int)(phibins[ie]   - minval_phi)/binw_phi + 1;
	    int hphi = hreco->GetXaxis()->FindBin(phibins[ie+1]);//(int)(phibins[ie+1] - minval_phi)/binw_phi + 1;
	    
	    TH1D *h1reco = (TH1D*)hreco->ProjectionZ(Form("hpz_reco_%d_%d_%d",nj,ip,ie),lphi,hphi,lbin,hbin,"e");
	    // if(nj==1){
	    ...
	    TH1D *h1raw = (TH1D*)hraw->ProjectionZ("hpz_raw",lphi,hphi,lbin,hbin,"e");
  ...
  TLine *line = new TLine(phibins[0],1.00,phibins[b2],1.00);
  line->SetLineWidth(2);
  line->SetLineStyle(2);
  line->SetLineColor(1);
  ...
  for(int in=0; in<kfiles; in++){
	...
	for(int nj=0; nj<knj;nj++){
	  ...
	  hArM_r[in][plot[nj]][ic][ip]->SetMarkerStyle(jsty[in]+4);
	  ...
	  hMean_r[in][plot[nj]][ic][ip]->SetMarkerStyle(jsty[in]+4);
	  ...
	  hArM[in][plot[nj]][ic][ip]->SetMaximum(1.05);      
	  hArM[in][plot[nj]][ic][ip]->SetMinimum(0.95);      
	  hMean[in][plot[nj]][ic][ip]->SetMaximum(1.05);      
	  hMean[in][plot[nj]][ic][ip]->SetMinimum(0.95);      

	  // hArM[in][plot[nj]][ic][ip]->SetMaximum(1.28);      
	  // hArM[in][plot[nj]][ic][ip]->SetMinimum(0.32);      
	  // hMean[in][plot[nj]][ic][ip]->SetMaximum(1.28);      
	  // hMean[in][plot[nj]][ic][ip]->SetMinimum(0.32);      
	  ...
	  if(ipad==2)l4->Draw();
	  line->Draw();
	  //if(ik!=1)c3[ik][ip]->Close();
	  c3[ik][ip]->SaveAs(Form("JESJER/phi/JESJER_phidep_%0.0fpT%0.0f_%s.gif",ptbins[ip],ptbins[ip+1],alg[ik]));
	}//! ip
      }//! icen
    }//! knj
  }//! in
...
{
  h1 = new TH1D(Form("%s_%s_%d_%d_%d",name,cver,nj,ic,ip),Form("CMSSW %s %s %s %s %d %d",cver,caption,ksp[ic],calgo[nj],ic,ip),nphi,phibins);
  MakeHist(h1,statop,"#phi","<p_{T}^{RecoJet}/p_{T}^{GenJet}>");
}
...
void MakeHistRMS(TH1 *h1,float ymax,float ymin)
{
  ...
  h1->GetXaxis()->SetTitle("GenJet #phi");
