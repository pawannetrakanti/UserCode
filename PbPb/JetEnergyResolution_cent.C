...
const char *algn [knj] = {"Anti-k_{T}, VS PF, R = 0.3","Anti-k_{T}, VS PF, R = 0.4","Anti-k_{T}, VS PF, R = 0.5",
                          ...
                          "Anti-k_{T}, PU PF, R = 0.3","Anti-k_{T}, PU PF, R = 0.4""Anti-k_{T}, PU PF, R = 0.5",
                          "Anti-k_{T}, PU Calo, R = 0.3","Anti-k_{T}, PU Calo, R = 0.4","Anti-k_{T}, PU Calo, R = 0.5",
};
...
const char *ksp  [ncen] = {"pp embed", "pp embed", "pp embed", "pp embed"};
...
double ptbins[]   = {30,40,50,60,70,80,90,100,110,120,130,140,150,160,170,180,190,200,240,286};
...
const int kfiles=1;
const char *version[kfiles] = {"53x-80-Track8-Jet21"};
const char *cleg[kfiles]    = {"Track8-Jet21"};
const int icol[kfiles] ={1};
const int isty[kfiles] ={20};
const int jsty[kfiles] ={20};

int JetEnergyResolution_cent(const char *reta = "eta2")
{
  ...
  for(int in=0; in<kfiles; in++){
    ...
    //fin_pbpb[in] = new TFile("input/JetResponse_histos_pp_embedded_0100.root","r");
    fin_pbpb[in] = new TFile("input/JetResponse_histos_pp_embedded_cent.root","r");
  ...
  for(int in=0; in<kfiles; in++){
    ipad=0;
    for(int nj=0; nj<knj;nj++){
      c3[nj] = new TCanvas(Form("c3_%d",nj),Form("%s JES JER",calgo[nj]),1528,656);
      makeMultiPanelCanvas(c3[nj],maxc,maxr,0.0,0.0,0.22,0.22,0.02,0);
      ipad=0;
      for(int ic=ncen-1;ic>=0;ic--){
	
	hRMS[in][nj][ic]->SetMarkerStyle(isty[in]);
	hRMS[in][nj][ic]->SetMarkerColor(icol[in]);
	hRMS[in][nj][ic]->SetLineColor(icol[in]);
	hRMS[in][nj][ic]->SetMarkerSize(1.3);

	hArM[in][nj][ic]->SetMarkerStyle(isty[in]);
	hArM[in][nj][ic]->SetMarkerColor(icol[in]);
	hArM[in][nj][ic]->SetLineColor(icol[in]);
	hArM[in][nj][ic]->SetMarkerSize(1.3);

	hArM_r[in][nj][ic]->SetMarkerStyle(jsty[in]);
	hArM_r[in][nj][ic]->SetMarkerColor(icol[in]);
	hArM_r[in][nj][ic]->SetLineColor(icol[in]);
	hArM_r[in][nj][ic]->SetMarkerSize(1.3);

	hSigma[in][nj][ic]->SetMarkerStyle(isty[in]);
	hSigma[in][nj][ic]->SetMarkerColor(icol[in]);
	hSigma[in][nj][ic]->SetLineColor(icol[in]);
	hSigma[in][nj][ic]->SetMarkerSize(1.3);

	hMean[in][nj][ic]->SetMarkerStyle(isty[in]);
	hMean[in][nj][ic]->SetMarkerColor(icol[in]);
	hMean[in][nj][ic]->SetLineColor(icol[in]);
	hMean[in][nj][ic]->SetMarkerSize(1.3);

	hMean_r[in][nj][ic]->SetMarkerStyle(jsty[in]);
	hMean_r[in][nj][ic]->SetMarkerColor(icol[in]);
	hMean_r[in][nj][ic]->SetLineColor(icol[in]);
	hMean_r[in][nj][ic]->SetMarkerSize(1.3);


	c3[nj]->cd(++ipad);
	//gPad->SetLogx();
	
	hRMS[in][nj][ic]->SetMaximum(0.532);
	hSigma[in][nj][ic]->SetMaximum(0.532);
	if(in==0)hRMS[in][nj][ic]->Draw("p");
	else hRMS[in][nj][ic]->Draw("psame");
	//if(in==0)hSigma[in][nj][ic]->Draw("p");
	//else hSigma[in][nj][ic]->Draw("psame");

	
	if(ipad!=1){
	  MakeZero(hRMS[in][nj][ic]);
	  MakeZero(hSigma[in][nj][ic]);
	  MakeZero(hMean[in][nj][ic]);
	  MakeZero(hArM[in][nj][ic]);
	}

	if(ipad==1){
	  drawText2("CMS Simulation", 0.28, 0.86, 22);
          drawText2("PYTHIA Z2 + HYDJET 1.8",0.28,0.75,21);
	  //drawText2("PYTHIA Z2",0.28,0.75,21);
	}
	else if(ipad==2){
	  drawText2("#sqrt{s_{NN}} = 2.76 TeV",0.45,0.88,21);
	  drawText2(Form("|#eta| < %0.0f",ketacut),0.15,0.88,21);
	  //l4->Draw();
	}
      
	drawText2(ccent[ic],0.75,0.82,21);
	drawText2(Form("%s",calgo[nj]),0.28,0.65,21);      
	...
	if(nj<2){
	  hArM[in][nj][ic]->SetMaximum(1.05);      
	  hArM[in][nj][ic]->SetMinimum(0.95);      
	  hMean[in][nj][ic]->SetMaximum(1.05);      
	  hMean[in][nj][ic]->SetMinimum(0.95);      
	}else if(nj<4){
	  hArM[in][nj][ic]->SetMaximum(1.184);      
	  hArM[in][nj][ic]->SetMinimum(0.95);      
	  hMean[in][nj][ic]->SetMaximum(1.184);      
	  hMean[in][nj][ic]->SetMinimum(0.95);      
	}else if(nj<6){
	  hArM[in][nj][ic]->SetMaximum(1.184);      
	  hArM[in][nj][ic]->SetMinimum(0.95);      
	...
	if(in==0){
	  hArM[in][nj][ic]->Draw("p");
	  //hArM_r[in][nj][ic]->Draw("psame");
	}else{
	  hArM[in][nj][ic]->Draw("psame");
	  //hArM_r[in][nj][ic]->Draw("psame");
	}

	// if(in==0){
	//   hMean[in][nj][ic]->Draw("p");
	//   //hMean_r[in][nj][ic]->Draw("psame");
	// }else{
	//   hMean[in][nj][ic]->Draw("psame");
	//   //hMean_r[in][nj][ic]->Draw("psame");
	// }
	if(ipad==2)l4->Draw();
	line->Draw();
...
void CalJesJer(const char *cver, int nj, int ic, TH2 *h2d, TH1F **h1d, TH1 *ham, TH1 *hr, TH1 *hm, TH1 *hs)
{
  for(int ip=0;ip<nbins;ip++){
    ...
    h1d[ip]  = (TH1F*)h2d->ProjectionY(Form("hratiocorrrefpt1D_%s_%d_%d_%d",cver,nj,ic,ip),lbin,hbin,"e");
