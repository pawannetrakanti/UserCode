...
float SimpleJetCorrectionUncertainty::uncertainty(std::vector<float> fX, float fY, bool fDirection) const 
{
  ...
  //if (bin<0)throw cms::Exception("SimpleJetCorrectionUncertainty")<<" bin variables out of range";
  if (bin<0)cout<<"SimpleJetCorrectionUncertainty"<<" bin variables out of range"<<endl;
  else result = uncertaintyBin((unsigned)bin,fY,fDirection);
  return result;
}
/////////////////////////////////////////////////////////////////////////
float SimpleJetCorrectionUncertainty::uncertaintyBin(unsigned fBin, float fY, bool fDirection) const 
{
  //if (fBin >= mParameters->size())throw cms::Exception("SimpleJetCorrectionUncertainty")<<" wrong bin: "<<fBin<<": only "<<mParameters->size()<<" are available";
  if (fBin >= mParameters->size())cout<<"SimpleJetCorrectionUncertainty"<<" wrong bin: "<<fBin<<": only "<<mParameters->size()<<" are available"<<endl;
  const std::vector<float>& p = mParameters->record(fBin).parameters();
  //if ((p.size() % 3) != 0)throw cms::Exception ("SimpleJetCorrectionUncertainty")<<"wrong # of parameters: multiple of 3 expected, "<<p.size()<< " got";

  if ((p.size() % 3) != 0)cout<<"SimpleJetCorrectionUncertainty"<<"wrong # of parameters: multiple of 3 expected, "<<p.size()<< " got"<<endl;
  ...
  if (fX[0] == fX[1])
    {
      if (fY[0] == fY[1])
        r = fY[0];
      else cout<<"SimpleJetCorrectionUncertainty"<<" interpolation error"<<endl;
      //else throw cms::Exception("SimpleJetCorrectionUncertainty")<<" interpolation error";
    } 
  else   
    {
