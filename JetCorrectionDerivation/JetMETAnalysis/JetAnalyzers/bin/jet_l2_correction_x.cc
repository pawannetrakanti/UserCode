    ...
    string alg(idir->GetName()); if (!contains(algs,alg)) continue;
     ...
     while ((hrsp=hl_rsp.next_object(indices))) {
		...
		fabscor->SetParLimits(2,0.1,100);
	       ...
	       // 	 cout<<" !!!!!!!!!!  xmax less than 15 " <<xmax<<endl;
               // }
             }
	       ...
	       fabscor=new TF1("fit","[0] + [1]/(pow(log10(x),[2])+[3]/pow(log10(x),2)) + [4]/x",xmin,xmax);
               fabscor->SetParameter(0,1.0);
               fabscor->SetParameter(1,5.0);
               fabscor->SetParameter(2,3.0);
               fabscor->SetParameter(3,3.0);
...
//        "(x>=15)*([0]+[1]*log10(x)+[2]*pow(log10(x),2)+[3]*pow(log10(x),3)+[4]*pow(log10(x),4))+ (x<15)*1";
     ...
     else  if (alg.find("Calo")!=string::npos)fnc_as_str   = "[0] + [1]/(pow(log10(x),[2])+[3]/pow(log10(x),2)) + [4]/x";
