...
int main(int argc,char**argv)
{
  ...
  while ((dirKey=(TKey*)nextDir())) {
    ...
    if(alg.find("Calo")!=string::npos)grsp->Fit(fitrsp,"R","",10.0,grsp->GetX()[grsp->GetN()-1]); 
    ...
    //gcor->Fit(fitcor,"QR");
    if(alg.find("Calo")!=string::npos)gcor->Fit(fitcor,"R","",10.0,gcor->GetX()[grsp->GetN()-1]);
    else gcor->Fit(fitcor,"R");
